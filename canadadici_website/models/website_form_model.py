...
class WebsiteFormModel(models.Model):
    ...
    def _get_form_writable_fields(self, property_origins=None):
        ...
        if self.model == "mail.mail":
            ...
            if self.model == "res.partner":
                included.update({'city_id'})
                included.update({'municipality_id'})
    ...
    def get_authorized_fields(self, model_name, property_origins):
        ...
        for field, value in fields_get.items():
            if field == 'city_id':
                _logger.error("City id trouvé")
            if field == 'municipality_id':
                _logger.error("Municipality id trouvé")
