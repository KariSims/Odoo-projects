...
class ReportController(http.Controller):
    ...
    def report_download(self, data, context=None, token=None, readonly=True):
        ...
        reportname = '???'
        try:
            # Ajout du paramètre aléatoire à l'URL
            random_param = str(uuid.uuid4())
            url_with_random = f"{url}&random={random_param}"
            ...
            if type_ in ['qweb-pdf', 'qweb-text']:
                ...
                reportname = url_with_random.split(pattern)[1].split('?')[0]
                ...
                if docids:
                    ...
                    random_param = str(uuid.uuid4())
                    url_with_random = f"{url}&random={random_param}"
                    data = url_parse(url_with_random).decode_query(cls=dict)
                    # Particular report:
