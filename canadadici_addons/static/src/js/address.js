...
publicWidget.registry.websiteSaleAddress = publicWidget.Widget.extend({
    ...
    init: function () {
        ...
        this._changeCity = debounce(this._changeCity.bind(this), 900);
    ...
    async _changeCountry(init=false) {
        ...
        if (!init || selectStates.options.length === 1) {
            ...
            if (data.states.length || data.state_required) {
                ...
                this.addressForm.municipality_id.value = ""
            
            } else {
                console.log("Je cache state_id")
                this._hideInput('state_id');
            }
        }

        // populate states and display
        var selectCities = this.addressForm.city_id;
        if (!init || selectCities.options.length === 1) {
            // dont reload state at first loading (done in qweb)
            if (data.cities.length || data.city_required) {
                // empty existing options, only keep the placeholder.
                selectCities.options.length = 1;

                // create new options and append them to the select element
                data.cities.forEach((city) => {
                    let option = new Option(city[1], city[0]);
    ...
    async _changeCity(init=false) {
        ...
        const data = await rpc(
            `/shop/city_info/${parseInt(CityId)}`,
            {address_type: this.addressType},
        );

        // populate states and display
        var selectMunicipalities = this.addressForm.municipality_id;
        if (!init || selectMunicipalities.options.length === 1) {
            // dont reload state at first loading (done in qweb)
            if (data.municipalities.length || data.municipality_required) {
                // empty existing options, only keep the placeholder.
                selectMunicipalities.options.length = 1;

                // create new options and append them to the select element
                data.municipalities.forEach((municipality) => {
                    let option = new Option(municipality[1], municipality[0]);
    ...
    _hideInput(name) {
        // show parent div, containing label and input
        console.log("hide input pourquoi?")
        ...
        if (!ev.defaultPrevented && !submitButton.disabled) {
            ...
            if (result.successUrl) {
                ...
                result.invalid_fields.forEach(
                    fieldName => console.log("fieldName: "+fieldName)
                );
