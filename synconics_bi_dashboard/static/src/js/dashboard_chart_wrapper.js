...
export class DashboardChartWrapper extends Component {
  static props = {
    ...
    reloadKey: Number,
    globalFilters: { optional: true, type: Object },
    onUpdateExport: { optional: true, type: Function },
    editMode: { optional: true, type: Boolean },
    onDeleteChart: { optional: true, type: Function },
  };
  ...
  setup() {
    ...
    this.onDeleteChartClick = () => {
      this.dialog.add(ConfirmationDialog, {
        title: _t("Supprimer le graphique"),
        body: _t("Voulez-vous vraiment supprimer ce graphique ?"),
        confirm: async () => {
          try {
            await this.orm.call("dashboard.chart", "unlink", [[parseInt(this.state.chartId)]]);
            this.notification.add(_t("Graphique supprimé avec succès"), { type: "success" });
            if (this.props.onDeleteChart) {
              this.props.onDeleteChart(this.state.chartId);
            }
          } catch (e) {
            this.notification.add(_t("Erreur lors de la suppression du graphique"), { type: "danger" });
          }
        },
        cancel: () => {},
      });
    ...
    this.update_chart = async (chartId, chartType, domain) => {
      let chart_data = await this.orm.call(
        ...
        [chartId],
        {
          ...
          global_filters: this.props.globalFilters || null,
        },
      );
  ...
  async update_record_sets(recordId, chart_type, isDirty, name, data, globalFilters) {
    let recordSets = await this.orm.call(
      ...
      [parseInt(recordId)],
      {
        ...
        isDirty,
        data,
        global_filters: globalFilters !== undefined ? globalFilters : (this.props.globalFilters || null),
      },
    );
    if (
