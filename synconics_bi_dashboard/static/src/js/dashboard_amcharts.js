...
import { GlobalFilterPanel } from "../components/GlobalFilterPanel/GlobalFilterPanel";
...
export class DashboardAmcharts extends Component {
  setup() {
    ...
    this.onApplyFilters = (filters) => {
      this.state.globalFilters = filters;
    };
    this.grid = false;
    ...
    this.onDeleteChart = (chartId) => {
      const el = document.querySelector(`.grid-stack-item[data-chart-id="${chartId}"]`);
      if (el && this.grid) {
        this.grid.removeWidget(el, false);
      }
      this.state.charts = this.state.charts.filter(c => c.id !== parseInt(chartId));
    };
...
DashboardAmcharts.components = { DashboardChartWrapper, GlobalFilterPanel };
